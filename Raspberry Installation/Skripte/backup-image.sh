...
mountpoint='/media/pi/TRANSFIX3'

#Path were the image of your SD card should be saved to
STORAGEPATH="/media/pi/TRANSFIX3"

# E-Mail Address
EMAIL="mail@e-mail.com"

# Location of your log file
LOGFILE="/var/log/backup_image.log"
...
sudo dd if=/dev/mmcblk0 of=${STORAGEPATH}/radio_$(date +%Y-%m-%d).img bs=1MB
...
#sudo dd if=/dev/mmcblk0 bs=1M | gzip - | dd of=${STORAGEPATH}/radio_$(date +%Y-%m-%d).gz
...
echo "Sent backup status via e-mail" | mail -a ${LOGFILE} -s "NEXTCLOUD Pi - Backup SUCCESSFULL" ${EMAIL} < ${LOGFILE}
